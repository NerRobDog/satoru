...
| Prime World | work in progress | [`games/prime-world`](games/prime-world/) → _(repo not published yet)_ | offline 5v5 vs bots plays (first time on a Mac at all); x87 client build + x87sidecar (an SSE2 rebuild is faster but breaks audio); the game server rebuilt from community sources runs under Wine (`pw-ci`, branch `server-build`). Built from the community sources under Nival's non-commercial licence; game files come from Nival's own public repository and no content of theirs is redistributed. Nival is a registered trademark of NIVAL INTERNATIONAL LIMITED; this project is not affiliated with or endorsed by Nival |
