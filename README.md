...
| Prime World | work in progress | [`games/prime-world`](games/prime-world/) → _(repo not published yet)_ | offline 5v5 vs bots plays (first time on a Mac at all); CI Release build, 99 % of time in x87 → SSE2 rebuild pending. Built from the community sources under Nival's non-commercial licence; game files come from Nival's own public repository and no content of theirs is redistributed. Nival is a registered trademark of NIVAL INTERNATIONAL LIMITED; this project is not affiliated with or endorsed by Nival |
