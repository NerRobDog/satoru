...
|---|---|---|---|
| Age of Empires IV | **release candidate v0.1** | [`games/aoe4`](games/aoe4/) → `<REPO-LINK>` | own Wine engine (LGPL source + relocator patch) + x87sidecar, no GameToMac; flat 60 on M1 Pro 16 GB and M5 Air, memory stays at 4.5–5.5 GB |
| Overwatch 2 | playable | [`games/ow2`](games/ow2/) → `<REPO-LINK>` | DXMT fork `ir-release`: shader-IR release patch takes the game from 13 GB to ~4 GB resident, swap gone |
| Magicka | work in progress | [`games/magicka`](games/magicka/) → `<REPO-LINK>` | FNA port (native, no Wine); tutorial playable, shadows and M5 testing open |
| Prime World | work in progress | [`games/prime-world`](games/prime-world/) → `<REPO-LINK>` | offline 5v5 vs bots plays (first time on a Mac at all); CI Release build, 99 % of time in x87 → SSE2 rebuild pending |
...
| Wine engine for AoE IV (`components/wine-aoe4`) | LGPL 2.1 | CrossOver 26.3 / Wine 11.0 tree with Marc Ibrahim's Rosetta patch (published with GameToMac 0.1.5-alpha) + our relocator diff — `<WINE-SOURCE-REPO-LINK>` |
| x87sidecar (`components/x87sidecar`) | MIT | fork of Lifeisawful/rosettax87_jit by athei — `<SIDECAR-SOURCE-REPO-LINK>` |
